...
// -----------------------------------------------------------------------------
kgSeldsp::kgSeldsp(void)
{
	_name    = "kgseldsp";
	_version = "###VERSION###";

	#include <help/en/kgseldspHelp.h>
	_titleL = _title;
	_docL   = _doc;
	#ifdef JPN_FORMAT
		#include <help/jp/kgseldspHelp.h>
	#endif
...
void kgSeldsp::sellist(kgTermiosEX *sc,char **selp ,int st,int pos)
{
...
int kgSeldsp::run(void) try
{
	...
	if(_height<=0 || cnt < _height ){ _height=cnt; }
	...
	_oFile.writeStr(selp[scrlpos+viewpos],true);
	_oFile.close();
