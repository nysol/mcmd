...
class kgTimeFormat{
	...
	kgTimeFormat(void):	_y(-1),_m(-1),_d(-1),_H(-1),_M(-1),_s(-1){}
	...
	kgstr_t str(int flg){
		...
		else if(flg == SEC)	 { sprintf(buf,"%d" ,_s);}
		else if(flg == TIME) { sprintf(buf,"%02d%02d%02d" ,_H,_M,_s);}
		else if(flg == DT)	 { sprintf(buf,"%04d%02d%02d%02d%02d%02d" ,_y,_m,_d,_H,_M,_s);}
