...
\n\
Commonly used examples are shown in - . In table , select records where\n\
val is \"y\" regardless of the value of key. In table , if any of the\n\
record contains the value \"x\" in val, records with the same key value\n\
will be selected. i.e. all records with value \"a\" in key column are\n\
selected. Since none of the records with key value \"b\" contains value\n\
\"x\", none records are selected.\n\
\n\
   key   val\n\
\n\
    a     x\n\
...
Use msel command to build complex conditions using regular expressions\n\
and operators which cannot be specified in this command.\n\
\n\
-   v= Match any character string from the list of string(s) specified.\n\
\n\
-   f= Match character string from the column(s) specified.\n\
\n\
-   AND operator can be used to match values multiple fields\n\
    (-F option).\n\
\n\
-   Matching for exact, start, middle or partial string can be specified\n\
    (-head,-tail,-sub option).\n\
\n\
-   k= Select records related to the defined key.\n\
\n\
-   Select records that matches all conditions by the key field\n\
    (-R option).\n\
\n\
Sample data with same key, containing two records and two columns is\n\
shown in (Table).\n\
\n\
    mselstr k=key f=fld1,fld2 v=s1,s2\n\
\n\
Matching criteria without -R,-F options are shown in .\n\
\n\
...
\n\
  ...
  ---- ---- ---------------------------------------------------------------------------------------------------------------------\n\
            ((va1 == s1 or va1 == s2) or (va2 == s1 or va2 == s2)) or ((vb1 == s1 or vb1 == s2) or (vb2 == s1 or vb2 == s2))\n\
   -F       ((va1 == s1 or va1 == s2) and (va2 == s1 or va2 == s2)) or ((vb1 == s1 or vb1 == s2) and (vb2 == s1 or vb2 == s2))\n\
        -R  ((va1 == s1 or va1 == s2) or (va2 == s1 or va2 == s2)) and ((vb1 == s1 or vb1 == s2) or (vb2 == s1 or vb2 == s2))\n\
   -F   -R  ((va1 == s1 or va1 == s2) and (va2 == s1 or va2 == s2)) and ((vb1 == s1 or vb1 == s2) and (vb2 == s1 or vb2 == s2))\n\
\n\
  : Using the input data shown in Table , the query results of the\n\
  command mselstr k=key f=fld1,fld2 v=v1,v2 with and without -R and F\n\
  options differs accordingly. If query matches all conditions, the\n\
  output will print all rows (2 rows), when there is no matched records,\n\
  no records will returned.\n\
\n\
Format\n\
\n\
